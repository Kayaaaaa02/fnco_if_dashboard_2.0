...
export const insert_data_query = (param) => {
    ...
    paraSet.forEach((e, idx) => {
        ...
        e.seeding_cntry = !!e.seeding_cntry ? `'${e.seeding_cntry}'` : `'KR'`; // 기본값 'KR'
        e.user_nm = !!e.user_nm ? `'${String(e.user_nm).replace(/'/g, "''")}'` : null;
        e.campaign_name = !!e.campaign_name ? `'${String(e.campaign_name).replace(/'/g, "''")}'` : null;
        if (idx == 0) {
            get_insert_params += `(${e.post_id},${e.post_url},${e.platform},${e.seeding_product},${e.keyword},${e.crawling_start_dt},${e.crawling_end_dt},${e.seeding_cost},${e.content_summary},${e.user_id},${e.seeding_cntry},${e.user_nm},${e.campaign_name})`;
        } else {
            get_insert_params += `,(${e.post_id},${e.post_url},${e.platform},${e.seeding_product},${e.keyword},${e.crawling_start_dt},${e.crawling_end_dt},${e.seeding_cost},${e.content_summary},${e.user_id},${e.seeding_cntry},${e.user_nm},${e.campaign_name})`;
        }
    });
    ...
    return {
        insertQuery: `
    insert into fnco_influencer.mst_post_ugc (
      post_id,
      post_url, 
      platform,
      seeding_product, 
