...
async function runGeminiAlignmentCheck({ campaign, brandDna, strategyKo, personas, desires, awareness, concepts, arcPhases }) {
  ...
  const prompt = buildAlignmentPrompt(inputSummary);
...
function buildAlignmentPrompt(input) {
  return `당신은 인플루언서 시딩 캠페인 전략의 정합성(Alignment)을 분석하는 전문가 AI입니다.
...
채점 시 실제 데이터를 면밀히 분석하세요. 데이터가 없거나 부족한 차원은 낮은 점수를 부여하세요.
