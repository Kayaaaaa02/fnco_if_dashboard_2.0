...
async def generate_beauty_reels(
    ...
    if cuts_need_image and source_images:
        ...
        try:
            ...
            # 시나리오에서 visual_style 가져오기 (없으면 기본값)
            visual_style = scenario.get("visual_style", "clean_bright")
    ...
    # ============================================================
    # Phase 4: TTS 나레이션 (ElevenLabs) — with timestamps
    # ============================================================
    tts_cost = 0
    tts_path = ""
    tts_alignment = None  # 글자별 타임스탬프 (Phase 6에서 사용)
    tts_speed_factor = 1.0  # 속도 조절 배율 (Phase 6에서 사용)
    combined_text = ""  # TTS 원본 텍스트 (Phase 6에서 사용)
    ...
    if enable_tts and current_video and narration:
        print(f"\n[Phase 4] ElevenLabs TTS (voice: {tts_voice}, with timestamps)...")
        try:
            from .tts import (
            ...
            # TTS 생성 + 타임스탬프 (mp3 -> wav 자동 변환)
            raw_tts_path = str(tts_dir / f"{tts_voice}_raw.wav")
            tts_result = generate_tts_for_voice_preset_with_timestamps(
                preset_name=tts_voice,
                text=combined_text,
                output_path=raw_tts_path,
            )
            raw_tts = tts_result["wav_path"]
            tts_alignment = tts_result.get("alignment")
            print(f"  TTS generated: {raw_tts}")
            if tts_alignment:
                print(f"  Alignment: {len(tts_alignment['characters'])} chars mapped")

            # alignment JSON 저장 (디버깅용)
            if tts_alignment:
                import json as _json

                align_path = tts_dir / "alignment.json"
                with open(align_path, "w", encoding="utf-8") as f:
                    _json.dump(tts_alignment, f, ensure_ascii=False, indent=2)
            ...
            if tts_speed_match:
                ...
                raw_dur = get_wav_duration(raw_tts)
                tts_speed_factor = raw_dur / video_dur if video_dur > 0 else 1.0
    ...
    if enable_subtitle and current_video and phrases and tts_path:
        ...
        try:
            ...
            # 이미지 자막(thumbnail)과 싱크 자막(reels)은 역할과 위치가 다름
            # - 이미지 자막: 프로모션 텍스트, 이미지 상단/중앙에 베이킹
            # - 싱크 자막: 나레이션 텍스트, 영상 하단에 오버레이
            # → 모든 컷에 싱크 자막을 표시한다 (겹침 없음)
            filtered_phrases = phrases

            # 타이밍 계산: alignment 기반 (정확) 또는 글자 수 비례 (fallback)
            tts_dur = get_audio_duration(tts_path)

            if tts_alignment and combined_text:
                # alignment 기반 정확한 타이밍
                from .video_subtitle import calculate_phrase_timings_from_alignment

                timings = calculate_phrase_timings_from_alignment(
                    phrases_per_cut=filtered_phrases,
                    alignment=tts_alignment,
                    combined_text=combined_text,
                    speed_factor=tts_speed_factor,
                )
                if not timings:
                    # alignment 매핑 실패 시 fallback
                    print(
                        "  [WARN] Alignment mapping failed, falling back to char-proportional"
                    )
