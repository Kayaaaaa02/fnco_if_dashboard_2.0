...
# ============================================================
# ElevenLabs alignment 기반 정확한 타이밍 계산
# ============================================================
def calculate_phrase_timings_from_alignment(
    phrases_per_cut: list,
    alignment: dict,
    combined_text: str,
    speed_factor: float = 1.0,
) -> list:
    """ElevenLabs alignment 데이터로 정확한 프레이즈별 타이밍을 계산한다.

    글자별 시작/종료 시점 데이터를 기반으로 프레이즈 경계의 정확한 타이밍을 추출한다.
    글자 수 비례 추정이 아닌 실제 발화 타이밍을 사용하므로 음성-자막 싱크가 정확하다.

    Args:
        phrases_per_cut: [["프레이즈1", "프레이즈2"], ...] 컷별 프레이즈 리스트
        alignment: ElevenLabs alignment dict
            {
                "characters": ["아", "침", ...],
                "character_start_times_seconds": [0.0, 0.12, ...],
                "character_end_times_seconds": [0.12, 0.23, ...],
            }
        combined_text: TTS에 전달된 원본 텍스트 (" ... " 구분자 포함)
        speed_factor: TTS 속도 조절 배율 (speed_adjust_tts 적용 시)
            원본보다 빠르면 >1.0, 느리면 <1.0

    Returns:
        [(start_sec, end_sec, text), ...] 형태의 타이밍 리스트
    """
    if not alignment or not alignment.get("characters"):
        # alignment 없으면 빈 리스트 — 호출자가 fallback 처리
        return []
    ...
    # alignment 텍스트 재구성 (공백/특수문자 포함)
    alignment_text = "".join(chars)

    timings = []

    for cut_phrases in phrases_per_cut:
        for phrase in cut_phrases:
            # combined_text에서 프레이즈 위치 찾기
            phrase_start_idx = _find_phrase_in_alignment(
                phrase, combined_text, alignment_text, chars, starts, ends
            )

            if phrase_start_idx is not None:
                start_char_idx, end_char_idx = phrase_start_idx
                ...
                # speed_factor 적용
                # speed_factor > 1이면 음성이 빨라짐 → 타임스탬프도 줄어듦
                if speed_factor > 0 and abs(speed_factor - 1.0) > 0.01:
                    start_time = start_time / speed_factor
                    end_time = end_time / speed_factor

                timings.append((start_time, end_time, phrase))
            else:
                # 프레이즈를 alignment에서 찾지 못한 경우
                print(f"  [WARN] Phrase not found in alignment: '{phrase[:20]}...'")

    if timings:
        print(
            f"[SUBTITLE] Alignment-based timings: {len(timings)} phrases "
...
def _find_phrase_in_alignment(
    ...
    """프레이즈의 alignment 인덱스 범위를 찾는다.

    combined_text에서 프레이즈를 찾고, 해당 위치의 alignment 인덱스를 반환한다.

    Args:
        phrase: 찾을 프레이즈 텍스트
        combined_text: TTS 원본 텍스트
        alignment_text: alignment characters를 합친 텍스트
        chars: alignment characters 리스트
        starts: character_start_times_seconds
        ends: character_end_times_seconds

    Returns:
        (start_char_idx, end_char_idx) 또는 None
    """
    # combined_text에서 프레이즈 위치 찾기
    phrase_clean = phrase.strip()
    idx = combined_text.find(phrase_clean)

    if idx < 0:
        # 공백 무시하고 재시도
        phrase_no_space = phrase_clean.replace(" ", "")
        combined_no_space = combined_text.replace(" ", "")
        idx_ns = combined_no_space.find(phrase_no_space)
        if idx_ns < 0:
            return None
        # 공백 제거 인덱스를 원본 인덱스로 변환
        count = 0
        idx = 0
        for i, c in enumerate(combined_text):
            if c != " ":
                if count == idx_ns:
                    idx = i
                    break
    ...
    # combined_text 인덱스 → alignment 인덱스 매핑
    # alignment characters는 공백/특수문자 건너뛸 수 있으므로 유연하게 매핑
    phrase_end_in_text = idx + len(phrase_clean)

    # alignment 인덱스에서 해당 범위의 실제 글자들을 찾기
    start_align_idx = None
    end_align_idx = None

    # alignment chars를 순회하며 combined_text 위치와 매핑
    text_cursor = 0
    for ai, ac in enumerate(chars):
        # alignment char가 combined_text의 현재 커서 위치와 매칭되는지 확인
        while text_cursor < len(combined_text) and combined_text[text_cursor] != ac:
            text_cursor += 1

        if text_cursor >= len(combined_text):
            break

        if text_cursor >= idx and start_align_idx is None:
            start_align_idx = ai

        if text_cursor < phrase_end_in_text:
            end_align_idx = ai

        text_cursor += 1

    if start_align_idx is not None and end_align_idx is not None:
        # alignment 범위 유효성 검사
        if start_align_idx < len(starts) and end_align_idx < len(ends):
