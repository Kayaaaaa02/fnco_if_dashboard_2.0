...
VISUAL_STYLE_PRESETS: Dict[str, Dict[str, str]] = {
    "clean_bright": {
        "lighting": "5500K daylight-balanced soft box, even diffused light from front-left, consistent ring light fill",
        "color_grade": "clean neutral color grade, minimal color cast, bright and airy, consistent white balance",
        "skin_tone": "natural healthy Korean skin tone, no warm/golden cast, consistent across all shots",
        "background_tone": "clean white or light neutral background, same tone throughout",
    },
    "warm_natural": {
        "lighting": "warm window light 4500K, soft golden hour feel, warm fill light",
        "color_grade": "warm but not yellow, soft warm tones, subtle film grain",
        "skin_tone": "warm glowing skin, healthy radiance, honey-toned",
        "background_tone": "warm neutral, soft beige or cream",
    },
    "cool_editorial": {
        "lighting": "cool-toned studio light 6500K, editorial fashion feel, sharp directional light",
        "color_grade": "slightly desaturated, cool-leaning editorial grade, magazine quality",
        "skin_tone": "porcelain cool-toned skin, high-fashion look, matte finish",
        "background_tone": "cool gray or blue-toned minimal studio",
    },
    "soft_pink": {
        "lighting": "soft diffused 5000K light with subtle pink gel, beauty ring light, flattering even fill",
        "color_grade": "soft pastel tones, subtle pink highlights, dreamy look",
        "skin_tone": "flawless dewy skin, soft peach undertone, glass skin effect",
        "background_tone": "soft pink or blush neutral, clean and feminine",
    },
}
...
def _build_startframe_prompt(
    ...
    Args:
        ...
        visual_style: 시각 스타일 프리셋 키 (기본 clean_bright)

    Returns:
        프롬프트 문자열
    """
    # 시각 스타일 프리셋 로드
    style = VISUAL_STYLE_PRESETS.get(visual_style, VISUAL_STYLE_PRESETS["clean_bright"])

    # 커스텀 프롬프트가 있어도 시각 일관성 블록은 항상 추가
    if cut.get("image_prompt"):
        custom = cut["image_prompt"]
        custom += f"""

VISUAL CONSISTENCY (CRITICAL - must be identical across ALL cuts in this series):
- Lighting: {style['lighting']}
- Color grade: {style['color_grade']}
- Skin tone: {style['skin_tone']}
- Background tone: {style['background_tone']}
- Same color temperature and white balance across all cuts
- Same post-processing look (same Instagram filter feel)
- This is ONE continuous beauty video — every frame must feel like the SAME shoot session
"""
        return custom
...
"""
    ...
    type_instructions = {
        ...
        "cta": (
            ...
            "CRITICAL: The product must look EXACTLY like the PRODUCT REFERENCE image — "
            "same packaging shape, same colors, same logo placement, same materials. "
            "Do NOT create a different or generic product design. "
            "Replicate the actual product appearance precisely. "
...
- Product must EXACTLY match the PRODUCT REFERENCE image (same shape, color, packaging, logo)
- Do NOT invent a different product design — replicate the reference product precisely
...
async def _generate_single_startframe(
    ...
    Args:
        ...
        visual_style: 시각 스타일 프리셋 (기본 clean_bright)
    ...
    cut_id = cut["id"]
    prompt = _build_startframe_prompt(cut, scenario, visual_style=visual_style)
    ...
    if face_images:
        ...
        for i, face_path in enumerate(face_images[:3]):
            ...
            img = load_image(face_path, max_size=2048)
    ...
    if product_images:
        parts.append(
            types.Part(
                text=(
                    "=== PRODUCT REFERENCE (CRITICAL — EXACT MATCH REQUIRED) ===\n"
                    "The product in the generated image MUST look EXACTLY like this reference:\n"
                    "- Same packaging shape, same colors, same logo, same materials\n"
                    "- Do NOT create a different or generic product design\n"
                    "- Copy the actual product appearance precisely from this reference"
                )
            )
        )
        for i, prod_path in enumerate(product_images[:3]):
            parts.append(types.Part(text=f"[Product {i+1} — replicate this EXACTLY]"))
            img = load_image(prod_path, max_size=2048)
            parts.append(_pil_to_part(img))
