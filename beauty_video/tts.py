...
def generate_tts_with_timestamps(
    ...
    """ElevenLabs TTS로 음성 파일(WAV) + 글자별 타임스탬프를 생성한다.

    convert_with_timestamps() API를 사용하여 글자별 시작/종료 시점을 반환한다.
    ...
    print(f"[TTS] Generating with timestamps: voice={voice_id[:8]}... model={model}")
    print(
    ...
    try:
        # convert_with_timestamps() — 글자별 타임스탬프 포함
        ...
        if hasattr(response, "alignment") and response.alignment:
            ...
            print(f"[TTS] Alignment: {len(alignment['characters'])} characters mapped")
        else:
            print("[TTS] No alignment data in response (fallback to char-proportional)")

    except Exception as e:
        raise RuntimeError(f"ElevenLabs TTS (with_timestamps) 생성 실패: {e}")
...
def generate_tts_for_voice_preset_with_timestamps(
    preset_name: str,
    text: str,
    output_path: str,
    model: str = DEFAULT_TTS_MODEL,
) -> dict:
    """프리셋으로 TTS + 타임스탬프를 생성한다.

    Args:
        preset_name: 프리셋 이름 (예: "nicole", "rachel")
        text: TTS 변환 텍스트
        output_path: 출력 WAV 파일 경로
        model: ElevenLabs 모델 ID

    Returns:
        {"wav_path": str, "alignment": dict or None}
    """
    if preset_name not in VOICE_PRESETS:
        available = list(VOICE_PRESETS.keys())
        raise KeyError(
            f"Unknown voice preset: '{preset_name}'. Available presets: {available}"
        )
    ...
    print(f"[TTS] Using preset '{preset_name}' (with timestamps): {preset['desc']}")
    return generate_tts_with_timestamps(
        voice_id=voice_id,
        text=text,
